...
function createPatchDiff(patch, oldFunctionCode, err = null) {
  ...
  const debugContext = config.debugContext || 30;
...
export function patchModules(modules) {
  if (config.inspectAll) {
    for (const moduleId in modules) {
      if (modules[moduleId].__chom_module) continue;
      const newContents = `(${modules[
        moduleId
      ].toString()}).apply(this, arguments)`;
      try {
        // DUMB HACK DUMB HACK
        chomNative.generateFunction(newContents, moduleId);
        modules[moduleId] = window.chomFunction;
      } catch (err) {
        logger.error(`Couldn't forcibly inspect ${moduleId}?`, err);
      }
    }
  }
